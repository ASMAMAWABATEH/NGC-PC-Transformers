...
class NGCTransformer:
    ...
    def process(self, obs, lab, adapt_synapses=True):
        ...
        if lab is not None:
            ...
            self.projection.eq_target.mask.set(jnp.zeros((self.batch_size * self.seq_len, self.vocab_size)))
        ...
        ## ════════════════════════════════
        ## Perform several E-steps — always run inference, regardless of adapt_synapses
        for ts in range(0, self.T):
            self.clamp_input(obs)
            if lab is not None:
                self.clamp_target(lab)
            else:
                self.output.e_out.mask.set(jnp.zeros((self.batch_size * self.seq_len, self.vocab_size)))
        ...
        ## ════════════════════════════════
        ## Perform M-step only when synaptic adaptation is enabled
        if adapt_synapses:
